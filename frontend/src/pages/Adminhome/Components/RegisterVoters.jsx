...
function RegisterVoters() {
  ...
  useEffect(() => {
    const fetchUsers = async () => {
      try {
        ...
        const unverifiedUsers = response.data.filter(user => user.role !== 'admin' && user.isVerified === false); // Exclude admins and show only unverified users
        setUsers(unverifiedUsers); // Store only unverified users in state
      } catch (err) {
  ...
  // Function to verify a user with confirmation
  const handleVerify = async (id) => {
    const confirm = window.confirm("Are you sure you want to verify this voter?");
    if (confirm) {
      try {
        await axios.put(`http://localhost:3000/user/verify/${id}`, { isVerified: true }); // Update verified to true in the database
        setUsers(users.filter(user => user._id !== id)); // Remove the verified user from the state
        alert("Voter has been verified successfully!");
      } catch (err) {
  ...
  return (
    <div className="container mx-auto my-8">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 flex items-center justify-between">
        <h2 className="text-3xl font-bold flex-1 text-center">Register Voters</h2>
